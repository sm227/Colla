...
const CalendarPageContent: React.FC = () => {
  ...
  const {
    ...
    updateTask,
    fetchTasks,
  } = useTasks(projectId || currentProject?.id);
  const { user } = useAuth();
  ...
  const fetchCalendarEvents = useCallback(async () => {
    ...
    try {
      ...
      const calendarResponse = await fetch(
        `/api/calendar${projectId ? `?projectId=${projectId}` : ""}`
      );
      ...
      const taskEvents = tasksData.map(
        (task: {
          id: number;
          title: string;
          description: string;
      ...
      // 공휴일 이벤트 포맷팅
      const holidayEvents = holidaysData.map(
        (holiday: { id: string; title: string; date: string }) => ({
          ...holiday,
          id: holiday.id,
  ...
  const { calendarDays, processedCalendarTasks, tasksByDate } = useMemo(() => {
    ...
    const assignRowsToTasks = (tasks: CalendarTask[]): CalendarTask[] => {
      ...
      for (const task of sortedTasks) {
        ...
        while (!foundRow) {
          ...
          for (
            let pos = task.startPosition;
            pos < task.startPosition + task.duration;
            pos++
          ) {
    ...
    tasks.forEach((task) => {
      ...
      if (taskStartDate <= calendarEnd && taskDue >= calendarStart) {
        ...
        const displayStart =
          taskStartDate < calendarStart ? calendarStart : taskStartDate;
    ...
    return {
      calendarDays: allCalendarDays,
      processedCalendarTasks: tasksWithRows,
      tasksByDate: tasksByDateMap,
    };
  }, [currentDate, tasks]);
  ...
  useEffect(() => {
    if (calendarView === "day" && isToday(selectedDate || currentDate)) {
      ...
      const scrollPosition = Math.max(
        0,
        currentHour * 64 + (currentMinute * 64) / 60 - 200
      ); // 현재 시간보다 조금 위로
  ...
  const handleDrop = (date: Date) => async (e: React.DragEvent) => {
    ...
    try {
      ...
      if (draggedTask.isCalendarEvent) {
        // 캘린더 일정인 경우:
        // 원래의 시작일과 종료일 가져오기
        const originalStartDate = draggedTask.startDate
          ? new Date(draggedTask.startDate)
          : new Date(draggedTask.createdAt);
        const originalEndDate = draggedTask.dueDate
        ...
        // 드래그한 날짜가 기간 안에 있는 경우 (가까운 쪽 변경)
          ...
          const diffToStart = Math.abs(
            droppedDate.getTime() - originalStartDate.getTime()
          );
          // 종료일과 드래그 날짜의 차이
          const diffToEnd = Math.abs(
            originalEndDate.getTime() - droppedDate.getTime()
          );
      ...
      // 에러 발생 시 사용자에게 알림
      alert(
        "일정 업데이트 중 오류가 발생했습니다. 새로고침 후 다시 시도해 주세요."
      );
    }
  };
  ...
  const handleDropToSidebar = async (e: React.DragEvent) => {
    ...
    try {
      ...
      alert(
        "일정 처리 중 오류가 발생했습니다. 새로고침 후 다시 시도해 주세요."
      );
    }
  };
    ...
    if (event.isCalendarEvent) {
      ...
      setEditingEvent({
        ...
        endDate: event.dueDate
          ? format(new Date(event.dueDate), "yyyy-MM-dd'T'HH:mm")
          : "",
  ...
  function getCalendarEventClasses() {
    if (!mounted)
      return "bg-indigo-500/10 text-indigo-700 border-indigo-500/20"; // 기본값
    if (theme === "dark") {
  ...
  return (
    <div className="flex h-screen bg-background text-foreground">
      ...
      {/* 중앙 메인 캘린더 */}
      <main
        className={`flex-1 w-full h-full flex flex-col p-4 bg-background text-foreground ${
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              onClick={() =>
                setActiveTab(
                  activeTab === "unscheduled" ? "none" : "unscheduled"
                )
              }
            ...
            <div className="grid grid-cols-7 flex-1 h-0 min-h-0 auto-rows-fr">
              {calendarDays.map((day, idx) => {
                ...
                return (
                  <div
                    key={day.toString()}
                    className={`relative border border-border p-2 h-full ${
                      !isCurrentMonth
                    ...
                    onDrop={(e) => {
                      ...
                      if (
                        draggedTask &&
                        !draggedTask.isCalendarEvent &&
                        !draggedTask.isHoliday
                      ) {
                        handleDrop(day)(e);
                      }
                    }}
                  >
                    ...
                    <div className="mt-6 space-y-1 relative h-full">
                      {(() => {
                        ...
                        const events = tasksByDate.get(dateKey) || [];
                        // 공휴일이 아닌 이벤트만 필터링
                        const nonHolidayEvents = events.filter(
                          (event) => !event.isHoliday
                        );
              ...
              >
                ...
                <thead className="sticky top-0 z-10">
                  <tr className="bg-muted text-muted-foreground border-b border-border">
                    <th
                      className="border-r border-border py-2 text-center text-sm font-medium"
                      style={{ width: "64px" }}
                    >
                      시간
                    </th>
                ...
                <tbody>
                  {Array.from({ length: 24 }).map((_, hour) => (
                    <tr key={hour} className="h-16">
                      <td
                        className="border-r border-border border-b border-border bg-muted/30 text-center align-top pt-1"
                        style={{ width: "64px" }}
                      >
                                ...
                                (() => {
                                  ...
                                  for (
                                    ...
                                    const conflictingEvents =
                                      finalEvents.filter((placedEvent) => {
                                        // 겹침 조건: 시간대가 겹치는 경우
                                        return (
                                          currentEvent.startMinute <
                                            placedEvent.endMinute &&
                                          currentEvent.endMinute >
                                            placedEvent.startMinute
                                        );
                                      });
                                  ...
                                  finalEvents.forEach((event) => {
                                    ...
                                    const overlappingEvents =
                                      finalEvents.filter(
                                        (otherEvent) =>
                                          event.startMinute <
                                            otherEvent.endMinute &&
                                          event.endMinute >
                                            otherEvent.startMinute
                                      );
                                  ...
                                  return finalEvents.map((processedTask) => {
                                    ...
                                    const leftOffset =
                                      totalColumns > 1
                                        ? `${
                                            (column * 95) / totalColumns + 2.5
                                          }%`
                                        : "2.5%";

                                    const start = processedTask.startDate
                                      ? new Date(processedTask.startDate)
                                      : undefined;
                                    const end = processedTask.endDate
                                      ? new Date(processedTask.endDate)
                                      : undefined;
                                    ...
                                    return (
                                      <div
                                        ...
                                        style={{
                                          ...
                                          overflow: "hidden",
                                          marginRight:
                                            totalColumns > 1 ? "2px" : "0px",
                                        }}
                                        onClick={() =>
                                          handleEventClick(processedTask)
                                        }
                                        draggable={
                                          !processedTask.isCalendarEvent &&
                                          !processedTask.isHoliday
                                        }
                                        onDragStart={
                                          !processedTask.isCalendarEvent &&
                                          !processedTask.isHoliday
                                            ? handleDragStart(processedTask)
                                            : undefined
                                        }
                                        onDragEnd={
                                          !processedTask.isCalendarEvent &&
        ...
        {calendarView === "day" && (
          <div className="bg-card text-card-foreground rounded-lg shadow border border-border overflow-hidden w-full h-full flex flex-col">
            <div className="border-b border-border text-center text-sm font-medium bg-muted text-muted-foreground py-2 flex items-center justify-center gap-2">
              <span>
                {format(selectedDate || currentDate, "yyyy년 MM월 dd일 (E)", {
                  locale: ko,
                })}
              </span>
              {(() => {
              ...
              <div className="flex">
                ...
                <div className="w-16 border-r border-border bg-muted/30">
                  {Array.from({ length: 24 }).map((_, hour) => (
                    <div
                      key={hour}
                      className="h-16 border-b border-border flex items-start justify-center pt-1"
                    >
                      <span className="text-xs text-muted-foreground">
                        {hour.toString().padStart(2, "0")}:00
                      </span>
                ...
                {(() => {
                  ...
                  if (nonHolidayEvents.length > 0) {
                    ...
                    timePoints.forEach((timePoint) => {
                      const overlappingCount = eventsWithTime.filter(
                        (event) =>
                          event.startMinute <= timePoint &&
                          event.endMinute > timePoint
                      ).length;
                      maxOverlapping = Math.max(
                        maxOverlapping,
                        overlappingCount
                      );
                    });
                    ...
                    >
                        ...
                        <div
                          ...
                          onDoubleClick={() => {
                            ...
                            setNewEvent({
                              ...
                              endDate:
                                format(clickedDate, "yyyy-MM-dd") +
                                `T${(hour + 1).toString().padStart(2, "0")}:00`,
                            });
                            setEditEventDialog({ show: false, event: null });
                          }}
                        />
                      ...
                      {(() => {
                        ...
                        const processedEvents = (() => {
                          ...
                          const eventsWithPosition = nonHolidayEvents.map(
                            (task, i) => {
                              const start = task.startDate
                                ? new Date(task.startDate)
                                : undefined;
                          ...
                          // 시간 순으로 정렬
                          const sortedEvents = [...eventsWithPosition].sort(
                            (a, b) => {
                              if (a.startMinute !== b.startMinute) {
                                return a.startMinute - b.startMinute;
                              }
                          ...
                          for (let i = 0; i < sortedEvents.length; i++) {
                            ...
                            // 현재 이벤트와 겹치는 이미 배치된 이벤트들 찾기
                            const conflictingEvents = finalEvents.filter(
                              (placedEvent) => {
                                // 겹침 조건: 시간대가 겹치는 경우
                                return (
                                  currentEvent.startMinute <
                            ...
                            // 사용 중인 컬럼 번호들 수집
                            const usedColumns = new Set(
                              conflictingEvents.map((event) => event.column)
                            );
                          ...
                          finalEvents.forEach((event) => {
                            // 현재 이벤트와 겹치는 모든 이벤트 찾기
                            const overlappingEvents = finalEvents.filter(
                              (otherEvent) =>
                        ...
                        return processedEvents.map((processedTask) => {
                          const { column, totalColumns, topPosition, height } =
                            processedTask;

                          // 디버깅용 로그
                          if (totalColumns > 1) {
                            console.log(
                              `일정 "${processedTask.title}": column=${column}, totalColumns=${totalColumns}`
                            );
                          }

                          // 컬럼 너비 계산 (겹치는 일정이 있으면 나누어서 표시)
                          const columnWidth =
                            totalColumns > 1 ? `${85 / totalColumns}%` : "85%";
                          const leftOffset =
                            totalColumns > 1
                              ? `${(column * 85) / totalColumns + 7.5}%`
                              : "7.5%";
                      ...
                      {isToday(selectedDate || currentDate) &&
                        (() => {
                          ...
                          const currentMinute = now.getMinutes();
                          const currentTimePosition =
                            currentHour * 64 + (currentMinute * 64) / 60;
                ...
                <div className="text-xs text-muted-foreground">
                  {editEventDialog.event?.user ? (
                    <>
                      ...
                      {editEventDialog.event.user.name}
                    </>
                  ) : editEventDialog.event?.userId ? (
                    <>
                      <span className="font-medium">생성자 ID:</span>{" "}
                      {editEventDialog.event.userId}
                    </>
                  ) : (
              ...
              {selectedTask.description && (
                <div>
                  ...
                  <div className="mt-1 text-sm text-foreground whitespace-pre-wrap">
                    {getSafeDescription(selectedTask.description)}
                  </div>
                </div>
              )}
          ...
          <>
            ...
            <div className="p-3 bg-muted/20 rounded-lg border-primary/50 ">
              <p className="text-xs text-muted-foreground leading-relaxed">
                <span className="font-medium text-foreground">팁:</span> 시간
                설정 없이 일정을 만들면 현재 시간으로 자동 설정됩니다
              </p>
            </div>
          </>
        )}
        ...
        >
          ...
          <div className="mb-4 p-3 bg-muted/50 rounded-lg border border-border">
            <p className="text-xs text-muted-foreground">
              💡 업무를 클릭하여 상세 정보를 보거나, 캘린더의 날짜로 드래그하여
              일정을 설정하세요
            </p>
          </div>
                  ...
                  return (
                    ...
                    >
                      <div className="flex justify-between items-start mb-2">
                        ...
                        <span
                          className={`text-xs px-2 py-1 rounded-full ${statusBg} ${statusText}`}
                        >
                          {task.status === "todo"
                            ? "할 일"
      ...
      <style jsx global>{`
        ...
        html:not(.dark)
          .unscheduled-tasks-scrollbar::-webkit-scrollbar-thumb:hover {
