...
export type Project = {
  ...
  createdAt: string;
  updatedAt: string;
  userId: string; // userEmail 대신 userId로 변경
};
...
export function ProjectProvider({ children }: { children: ReactNode }) {
  ...
  const fetchProjects = async () => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(data.error || "프로젝트를 불러오는 중 오류가 발생했습니다.");
      }
  ...
  const createProject = async (name: string, description?: string): Promise<Project> => {
    try {
      ...
      if (!response.ok) {
        const data = await response.json();
        throw new Error(data.error || "프로젝트를 생성하는 중 오류가 발생했습니다.");
      }
  ...
  useEffect(() => {
    ...
    // 상태 초기화 로직만 유지
    if (!loading && !authLoading && user && initialized && projects.length === 0) {
